...
exports.khaAgent = exports.KhaAgent = void 0;
...
const promptPrimedSessions = new Set();
...
function logRunItems(result) {
    ...
    for (const item of items) {
        ...
        if (rawType === 'function_call_result' && toolName) {
            console.log(`[KhaAgent] 🛠️ Tool Output: ${toolName} => ${truncateText(stringifySafe(maybeOutput))}`);
            continue;
        }
        if (toolName && (type.includes('tool') || type.includes('function') || type.includes('call'))) {
            console.log(`[KhaAgent] 🛠️ Tool Call: ${toolName} args=${truncateText(stringifySafe(maybeArgs))}`);
        ...
        if (type.includes('handoff')) {
            console.log(`[KhaAgent] 🔀 Handoff: ${agentName || 'unknown-agent'} (${truncateText(stringifySafe(item))})`);
            continue;
        }
...
function collectToolsCalled(result) {
    const items = Array.isArray(result?.newItems) ? result.newItems : [];
    const tools = new Set();
    for (const item of items) {
...
class KhaAgent {
     ...
     */
    async processMessage(input, chatwootId, context = {}) {
        try {
            ...
            if (!USE_STATELESS_SESSION) {
                ...
                if (!session) {
                    session = new agents_1.MemorySession();
            ...
            let result;
            try {
                result = await (0, agents_1.run)(agents_2.triageAgent, finalInput, {
                    ...(session ? { session } : {}),
                    context: runContext
                });
            }
                ...
                if (runError?.status === 400 && (runError?.message?.includes("'reasoning'") || runError?.message?.includes("'history'"))) {
                    ...
                    session = USE_STATELESS_SESSION ? undefined : new agents_1.MemorySession();
                    if (session)
                        activeSessions.set(chatwootId, session);
                    promptPrimedSessions.delete(chatwootId);
                    ...
                    promptPrimedSessions.add(chatwootId);
                    result = await (0, agents_1.run)(agents_2.triageAgent, retryFinalInput, {
                        ...(session ? { session } : {}),
                        context: runContext
                    });
                }
            ...
            return {
                ...
                logs: {
                    ...
                    usage: result.usage,
                    toolsCalled,
                    toolTelemetry: (0, tools_1.popToolTelemetry)(chatwootId)
                }
            };
        }
        catch (error) {
     ...
     */
    async clearSession(chatwootId) {
        activeSessions.delete(chatwootId);
        promptPrimedSessions.delete(chatwootId);
        console.log(`[KhaAgent] Sessão limpa para: ${chatwootId}`);
    }
}
exports.KhaAgent = KhaAgent;
exports.khaAgent = new KhaAgent();
