...
class McpClient {
     ...
     */
    constructor(url, name, authToken, tokenInUrl = true, transport = 'sse') {
        this.baseUrl = url;
        this.name = name;
    ...
    connectSSE() {
        ...
        try {
            ...
            const es = new eventsource_1.EventSource(connectUrl, esOptions);
            this.eventSource = es;
            es.onopen = () => {
    ...
    async performSSEHandshake() {
        ...
        try {
            ...
            console.log(`[MCP ${this.name}] Server initialized. Protocol: ${protocolVersion}`);
            console.log(`[MCP ${this.name}] Sending 'notifications/initialized'...`);
            await axios_1.default.post(this.sessionEndpoint, {
                jsonrpc: '2.0',
                method: 'notifications/initialized'
            }, {
                headers: this.getHeaders(),
    ...
    async sendSSERequest(method, params, timeoutMs = 30000) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                const response = await axios_1.default.post(this.sessionEndpoint, payload, {
                    headers: this.getHeaders(),
                    timeout: timeoutMs,
     ...
     */
    parsePostResponse(data, contentType) {
        if (!data)
            return null;
    ...
    async callTool(toolName, args) {
        ...
        try {
            ...
            const isSessionError = isTimeout ||
                status === 401 ||
                status === 400 ||
     ...
     */
    async callStreamable(method, params) {
        const requestId = `${Date.now()}-${Math.floor(Math.random() * 1000)}`;
        const payload = {
            jsonrpc: '2.0',
            method,
