...
exports.extractReservationFacts = extractReservationFacts;
...
const userLatestTimestamp = new Map();
...
function isInActiveFlow(from) {
    ...
    if (state.lastInteractiveMenu && activeMenus.has(state.lastInteractiveMenu))
        return true;
    return false;
}
function setLastInteractiveMenu(to, menu) {
...
async function postGraphMessageWithRetry(payload, label, retries = 2) {
    let attempt = 0;
    let lastErr = null;
...
function normalizeUnitNameForMatch(value) {
    return (value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
}
function isPromptInjectionLike(text) {
...
function isCardapioIntent(text) {
    const t = text.toLowerCase();
    const keywords = [
...
function isGenericCardapioRequest(text) {
...
function extractDeliveryCity(text) {
    const t = text.toLowerCase();
    if (t.includes('londrina'))
        return 'Londrina';
    if (t.includes('são paulo') || t.includes('sao paulo') || t.includes('sp'))
        return 'São Paulo';
    if (t.includes('curitiba') || t.includes('cwb'))
        return 'Curitiba';
    return null;
}
function formatBrazilPhoneDisplay(raw) {
...
function extractReservationFacts(text) {
    const lower = text.toLowerCase();
    const facts = {};
    const peoplePatterns = [/\b(\d{1,2})\s*pessoas?\b/i, /\bsomos\s*(\d{1,2})\b/i, /\bpara\s*(\d{1,2})\s*(pessoas?)?\b/i];
    for (const p of peoplePatterns) {
        const m = text.match(p);
        if (m?.[1]) {
            facts.people = parseInt(m[1], 10);
            break;
        }
    }
    if (/\bsem (filhos|criancas|crianças)\b/i.test(lower) || /\b0 (criancas|crianças|kids)\b/i.test(lower)) {
        facts.kids = 0;
    }
    else {
        const kidsMatch = text.match(/\b(\d{1,2})\s*(criancas|crianças|kids)\b/i);
        if (kidsMatch?.[1])
            facts.kids = parseInt(kidsMatch[1], 10);
    }
    ...
    if (!facts.dateText) {
        for (const [key, value] of Object.entries(weekdays)) {
            if (lower.includes(key)) {
                facts.dateText = value;
                break;
            }
        }
    }
    // Name extraction: first comma-separated chunk that looks like a name
    const chunks = text.split(/[,;]/)
        .map(c => c.trim())
        .filter(c => c.length > 0);
    for (const chunk of chunks) {
        const looksLikeGreeting = /^(oi|ol[áa]|bom dia|boa tarde|boa noite|tudo bem|blz|beleza|opa)\b/i.test(chunk);
        const isAffirmation = /^(sim|nao|não|ok|certo|isso|pode|fechado|show|perfeito|obrigado|valeu|confirmar)\b/i.test(chunk);
        const hasNumbers = /\d/.test(chunk);
        const hasReservaWords = /\b(reserva|quero|hoje|amanh|pessoas|hor[aá]rio|crian[çc]as|mudar|alterar|corrigir|cancelar|favor|minha)\b/i.test(chunk);
        if (!looksLikeGreeting && !isAffirmation && !hasNumbers && !hasReservaWords &&
            /^[A-Za-zÀ-ÿ'\s]{2,50}$/.test(chunk)) {
            facts.name = chunk;
            break;
        }
    }
...
function normalizeInteractiveCommand(raw, lastInteractiveMenu) {
    ...
    if (command === 'reservar mesa')
        return 'menu_reserva';
    if (command === 'ver delivery' || command === 'delivery')
        return 'menu_delivery';
    if (command === 'menu principal' || command === 'voltar ao menu')
        return 'MENU_PRINCIPAL';
    if (command === 'ver cardápio' || command === 'ver cardapio')
        return 'menu_cardapio';
    // Context-aware title fallback
    if (lastInteractiveMenu === 'delivery_choice') {
        if (plain.includes('novo pedido'))
            return 'delivery_novo';
        if (plain.includes('preciso de ajuda') || plain.includes('preciso ajuda'))
            return 'delivery_ajuda';
    }
    // Unit slug detection
    const unitSlug = plain.includes('jardim botanico') || plain === 'botanico' ? 'botanico' :
    ...
    if (unitSlug && lastInteractiveMenu === 'reserva_units')
        return `unidade_${unitSlug}`;
    if (lastInteractiveMenu === 'cardapio_cities') {
        if (plain.includes('curitiba'))
            return 'cardapio_curitiba';
        if (plain.includes('londrina'))
            return 'cardapio_londrina';
        if (plain.includes('sao paulo'))
            return 'cardapio_saopaulo';
    }
    if (lastInteractiveMenu === 'delivery_cities') {
        if (plain.includes('curitiba'))
            return 'delivery_curitiba';
        if (plain.includes('londrina'))
            return 'delivery_londrina';
        if (plain.includes('sao paulo'))
            return 'delivery_saopaulo';
    }
    if (lastInteractiveMenu === 'reserva_confirmation') {
        if (plain.includes('confirm_reserva_sim') || plain.includes('sim'))
            return 'confirm_reserva_sim';
        if (plain.includes('confirm_reserva_nao') || plain.includes('nao') || plain.includes('mudar'))
            return 'confirm_reserva_nao';
    }
    return original;
}
function normalizeFreeTextToCommand(text, state) {
    ...
    if (!awaitingConfirmation)
        return text;
    if (['confirmar', 'sim', 'ok', 'fechado'].includes(normalized))
        return 'confirm_reserva_sim';
    if (['não', 'nao', 'mudar', 'alterar', 'corrigir'].includes(normalized))
        return 'confirm_reserva_nao';
    return text;
}
function normalizeNaturalLanguageToMenuCommand(text, lastInteractiveMenu, messageType = 'text', from) {
    ...
    if (!fromInteractive) {
        if (/\b(delivery|entrega)\b/.test(flat) && /\b(quero|fazer|pedir|ver|menu)\b/.test(flat))
            return 'menu_delivery';
        if (/^(ver delivery|delivery|menu delivery)$/.test(flat))
            return 'menu_delivery';
        if (/\b(reserv)\b/.test(flat) && /\b(quero|fazer|uma|mesa|menu)\b/.test(flat))
            return 'menu_reserva';
        if (/^(reservar mesa|reservar|menu reserva|fazer reserva)$/.test(flat))
            return 'menu_reserva';
        if (/\b(cardapio|cardápio)\b/.test(flat))
            return 'menu_cardapio';
        if (/^(ver cardapio|ver cardápio)$/.test(flat))
            return 'menu_cardapio';
        if (/^(1|1️⃣)$/.test(flat))
            return 'menu_cardapio';
        if (/^(2|2️⃣)$/.test(flat))
    ...
    if (lastInteractiveMenu === 'after_cardapio') {
        if (/^(1|1️⃣)\b/.test(flat))
            return 'menu_reserva';
        if (/^(2|2️⃣)\b/.test(flat))
            return 'menu_delivery';
        if (/^(3|3️⃣)\b/.test(flat))
            return 'MENU_PRINCIPAL';
    }
    if (lastInteractiveMenu === 'delivery_choice') {
        if (/^(1|1️⃣)\b/.test(flat))
    ...
    if (/^(3|3️⃣)\b/.test(flat))
        return 'menu_delivery';
    if (flat.includes('ver delivery'))
        return 'menu_delivery';
    if (flat.includes('menu principal') || flat.includes('voltar'))
        return 'MENU_PRINCIPAL';
    if (flat.includes('ver cardápio') || flat.includes('ver cardapio'))
        return 'menu_cardapio';
    return text;
}
function shouldBypassContentDedup(text, messageType) {
    if (messageType === 'interactive' || messageType === 'button')
        return true;
    const t = (text || '').trim().toLowerCase();
    if (!t)
        return true;
    return (t === 'menu_principal' ||
        /^(1|2|3)\s*$/.test(t) ||
...
function shouldForceDeterministicRoute(text, messageType, from) {
    const t = String(text || '').trim().toLowerCase();
    if (!t)
        return false;
    if (messageType === 'interactive' || messageType === 'button')
        return true;
    if (MENU_COMMANDS.has(text.trim()))
        return true;
    // Explicit menu commands ALWAYS force deterministic route
    if (EXPLICIT_MENU_COMMANDS.has(t))
        return true;
    ...
    if (/^(1|2|3)\s*$/.test(t))
        return true;
    return (t.startsWith('menu_') || t.startsWith('cardapio_') || t.startsWith('delivery_') ||
        t.startsWith('unidade_') || t.startsWith('confirm_') || t.startsWith('phone_'));
}
// ─── Deterministic Menu Handlers ────────────────────────────────────────────
async function handleDeterministicMenuCommand(text, from, userName) {
    const normalized = text.trim().toLowerCase();
    ...
    if (command === 'MENU_PRINCIPAL') {
        ...
        await chatwoot_1.chatwootService.syncMessage(from, userName, 'MENU_PRINCIPAL', 'outgoing', { source: 'bot' });
        return true;
    }
    if (command === 'menu_cardapio') {
        ...
        currentState.hasInteracted = true;
        userContextState.set(from, currentState);
        await chatwoot_1.chatwootService.syncMessage(from, userName, 'MENU_CIDADES_CARDAPIO', 'outgoing', { source: 'bot' });
        return true;
    }
    if (command === 'menu_reserva') {
        ...
        await chatwoot_1.chatwootService.syncMessage(from, userName, 'LIST_RESERVA_UNIDADES', 'outgoing', { source: 'bot' });
        return true;
    }
    if (command === 'menu_delivery') {
        ...
        currentState.hasInteracted = true;
        userContextState.set(from, currentState);
        await chatwoot_1.chatwootService.syncMessage(from, userName, 'MENU_DELIVERY_CIDADES', 'outgoing', { source: 'bot' });
        return true;
    }
    return false;
}
async function handleDeterministicInteractiveCommand(text, from, userName) {
    ...
    if (CARDAPIO_CITY_COMMANDS.has(command)) {
        ...
        const link = await db_1.db.getConfig(keyMap[command]);
        const fallback = await db_1.db.getConfig('link_cardapio_curitiba');
        const finalLink = link || fallback || 'https://cardapio.kharina.com.br/';
        state.preferredCity = city;
    ...
    if (DELIVERY_CITY_COMMANDS.has(command)) {
        ...
        await chatwoot_1.chatwootService.syncMessage(from, userName, 'MENU_DELIVERY_OPCOES', 'outgoing', { source: 'bot' });
        return true;
    }
    // ── Delivery Novo / Ajuda ──
    if (command === 'delivery_novo') {
        ...
        if (city === 'Londrina') {
            ...
            await sendWhatsAppText(from, msg);
            await chatwoot_1.chatwootService.syncMessage(from, userName, msg, 'outgoing', { source: 'bot', flow: 'delivery' });
            await sendInteractiveMenu(from, true);
            return true;
        }
        const msg = [
    ...
    if (command === 'delivery_ajuda') {
        ...
        await chatwoot_1.chatwootService.syncMessage(from, userName, privateNote, 'outgoing', { source: 'bot', flow: 'delivery_help' }, true);
        if (city === 'Londrina') {
            const phone = await db_1.db.getConfig('phone_londrina');
            const msg = [
            ...
            await sendWhatsAppText(from, msg);
            await chatwoot_1.chatwootService.syncMessage(from, userName, msg, 'outgoing', { source: 'bot', flow: 'delivery_help' });
            await sendInteractiveMenu(from, true);
            return true;
        }
        const msg = [
    ...
    if (command === 'phone_use_current') {
        ...
        if (!reservation.dateText)
            missing.push('data');
        if (!reservation.timeText)
            missing.push('horário');
        if (reservation.people === undefined)
            missing.push('quantidade de pessoas');
    ...
    if (command === 'confirm_reserva_nao') {
        if (state.reservation)
            state.reservation.awaitingConfirmation = false;
        userContextState.set(from, state);
...
const handleWhatsAppWebhook = async (req, res) => {
    ...
    enqueueUserJob(message.from, async () => {
        ...
        if (!bypassDedup) {
            const isDupContent = await redis_1.redisService.isDuplicateContent(from, text.trim().toLowerCase());
            if (isDupContent)
                return;
        }
        try {
            ...
            if (st.reservation?.awaitingNewPhone && !/^menu_|^unidade_|^cardapio_|^delivery_|^confirm_/.test(text.trim().toLowerCase())) {
                ...
                const msg = `Perfeito! Vou usar o número ${formatBrazilPhoneDisplay(normalizedPhone)} para a reserva. ✅`;
                await sendWhatsAppText(from, msg);
                await chatwoot_1.chatwootService.syncMessage(from, userName, msg, 'outgoing', { source: 'bot' });
                return;
            }
            ...
            if (isPromptInjectionLike(text)) {
                ...
                await chatwoot_1.chatwootService.syncMessage(from, userName, INJECTION_BLOCK_MSG, 'outgoing', { source: 'bot' });
                return;
            }
            ...
            if (Object.keys(facts).length > 0) {
                ...
                if (facts.people !== undefined)
                    reservation.people = facts.people;
                if (facts.kids !== undefined)
                    reservation.kids = facts.kids;
                if (facts.occasion !== undefined)
                    reservation.occasion = facts.occasion;
                if (facts.notes !== undefined)
                    reservation.notes = facts.notes;
                if (facts.name !== undefined)
                    reservation.name = facts.name;
                if (facts.dateText !== undefined)
                    reservation.dateText = facts.dateText;
                if (facts.timeText !== undefined)
                    reservation.timeText = facts.timeText;
            ...
            // AI Agent processing
            const systemPrompt = await promptService_1.promptService.getSystemPrompt();
            const history = await redis_1.redisService.getHistory(from);
            const userState = userContextState.get(from) || {};
            const reservationState = userState.reservation || {};
            const mandatoryRules = [];
            if (reservationState.people !== undefined)
                mandatoryRules.push(`O cliente ja informou ${reservationState.people} pessoas.`);
            if (reservationState.kids !== undefined)
                mandatoryRules.push(`O cliente ja informou ${reservationState.kids} criancas.`);
            ...
            if (userState.preferredUnitName)
                mandatoryRules.push(`Unidade escolhida: ${userState.preferredUnitName}.`);
            if (userState.preferredStoreId)
                mandatoryRules.push(`storeId: ${userState.preferredStoreId}.`);
            if (reservationState.name)
                mandatoryRules.push(`Nome: ${reservationState.name}.`);
            if (reservationState.dateText)
                mandatoryRules.push(`Data: ${reservationState.dateText}.`);
            if (reservationState.timeText)
                mandatoryRules.push(`Horario: ${reservationState.timeText}.`);
            ...
            const result = await agent_1.khaAgent.processMessage(text, from, {
                history,
                systemPrompt,
                clientPhone: from,
            ...
            if (response) {
                ...
                if (matchedTrigger) {
                    ...
                    if (!matchedTrigger.menuOnly) {
                        ...
                        if (preamble) {
                            ...
                            await chatwoot_1.chatwootService.syncMessage(from, userName, safePreamble, 'outgoing', { source: 'bot' });
                        }
                    }
                ...
                await redis_1.redisService.saveHistory(from, history);
...
async function sendCardapioByCity(from, userName, city) {
    const keyMap = {
        Curitiba: 'link_cardapio_curitiba', Londrina: 'link_cardapio_londrina', 'São Paulo': 'link_cardapio_sp'
    };
    const link = await db_1.db.getConfig(keyMap[city]);
...
// ─── Interactive Menus ──────────────────────────────────────────────────────
async function sendInteractiveMenu(to, compact = false) {
    const menuPayload = {
    ...
    await sendInteractiveWithFallback(to, menuPayload, 'send_main_menu', 'Escolha: 1) Ver Cardápio 2) Reservar Mesa 3) Delivery');
...
async function sendCitiesMenu(to) {
    ...
    await sendInteractiveWithFallback(to, menuPayload, 'send_cities_menu', 'Escolha a cidade: Curitiba, Londrina ou São Paulo.');
...
async function sendUnidadesMenu(to) {
    ...
    await sendInteractiveWithFallback(to, menuPayload, 'send_unidades_menu', 'Qual unidade? (Jardim Botânico, Cabral, Água Verde, Batel, Portão, Londrina ou São Paulo).');
    setLastInteractiveMenu(to, 'reserva_units');
}
async function sendPhoneNumberConfirmation(to) {
    const menuPayload = {
        messaging_product: "whatsapp", to, type: "interactive",
        interactive: {
            type: "button",
    ...
    await sendInteractiveWithFallback(to, menuPayload, 'send_phone_confirmation', 'Posso usar este número para a reserva? Responda "sim" ou "não, usar outro".');
    setLastInteractiveMenu(to, 'phone_confirmation');
}
async function sendConfirmationMenu(to) {
    const menuPayload = {
        messaging_product: "whatsapp", to, type: "interactive",
        interactive: {
            type: "button",
            body: { text: "Os dados da reserva estão corretos? 😄" },
            action: {
                buttons: [
    ...
    await sendInteractiveWithFallback(to, menuPayload, 'send_confirmation_menu', 'Os dados da reserva estão corretos? Responda "sim" ou "não, mudar algo".');
...
async function sendDeliveryCitiesMenu(to) {
    ...
    const sent = await sendInteractiveWithFallback(to, menuPayload, 'send_delivery_cities_menu', 'De qual cidade? 1) Curitiba 2) Londrina 3) São Paulo');
...
async function sendDeliveryChoiceMenu(to) {
    ...
    await sendInteractiveWithFallback(to, menuPayload, 'send_delivery_choice_menu', 'Você quer: "Novo Pedido" ou "Preciso de Ajuda"?');
