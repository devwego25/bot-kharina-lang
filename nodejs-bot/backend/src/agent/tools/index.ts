...
const toolTelemetryByChatId = new Map<string, ToolTelemetryEvent[]>();

function pushToolTelemetry(chatwootId: string | undefined, event: ToolTelemetryEvent) {
...
export function popToolTelemetry(chatwootId: string): ToolTelemetryEvent[] {
...
export const cardapioMcp = new McpClient(
    process.env.MCP_CARDAPIO_URL || 'http://172.17.0.1:3001/mcp',
    'Cardapio',
    process.env.MCP_CARDAPIO_TOKEN || process.env.MCP_RESERVAS_TOKEN,
    true,  // tokenInUrl
    'sse'  // transport
);

export const reservasMcp = new McpClient(
    process.env.MCP_RESERVAS_URL || 'https://mcp.reservas.wegosb.com.br/mcp',
    'Reservas',
    process.env.MCP_RESERVAS_TOKEN,
    false,       // tokenInUrl: no ?token= in URL
    'streamable' // transport: direct POST (no SSE)
);

// ─── Tool Registry ─────────────────────────────────────────────────────

const toolRegistry: Record<string, { client: McpClient | null, localFn?: Function }> = {};

let cachedTools: OpenAI.ChatCompletionTool[] | null = null;
let lastFetchTime = 0;
const CACHE_TTL = 30 * 60 * 1000; // 30 minutes

// ─── Static/local tools (always available) ─────────────────────────────

const BASE_TOOLS: OpenAI.ChatCompletionTool[] = [
    {
        type: 'function',
        function: {
            name: 'mcp_adm',
...
toolRegistry['mcp_adm'] = {
    client: null,
    localFn: async (args: any) => {
        console.log('[Agent] Calling mcp_adm (Admin Webhook)', args);
        try {
            await axios.post('https://auto.webhook.kharina.com.br/webhook/0311f366-9d47-47f0-9802-e0fce06bec2a', {
                setor: args.setor,
                phone: args.phone,
...
toolRegistry['get_kids_info'] = {
    ...
    localFn: async () => {
        try {
            const content = await db.getConfig('kids_info_content');
            if (!content) return "Não há informações cadastradas sobre o Espaço Kids no momento.";
            return content;
        } catch (e: any) {
...
toolRegistry['get_cardapio_link'] = {
    client: null,
    localFn: async (args: any) => {
        const linkCuritiba = await db.getConfig('link_cardapio_curitiba');
        const linkLondrina = await db.getConfig('link_cardapio_londrina');
        const linkSP = await db.getConfig('link_cardapio_sp');
        ...
        if (loc.includes('londrina')) key = 'link_cardapio_londrina';
        else if (loc.includes('paulo')) key = 'link_cardapio_sp';

        const link = await db.getConfig(key);
...
async function fetchMcpTools(): Promise<OpenAI.ChatCompletionTool[]> {
    const mcpTools: OpenAI.ChatCompletionTool[] = [];
    const clients = [cardapioMcp, reservasMcp];
    ...
    for (const client of clients) {
        ...
        try {
            ...
            if (result?.tools) {
                ...
                const toolNames = result.tools.map((t: any) => t.name).join(', ');
    ...
    if (!toolRegistry['mcp_enviacardapio']) {
        toolRegistry['mcp_enviacardapio'] = { client: cardapioMcp };
    }
    if (!toolRegistry['mcp_reserva']) {
        toolRegistry['mcp_reserva'] = { client: reservasMcp };
    }
 ...
 */
export async function getToolsForAgent(): Promise<OpenAI.ChatCompletionTool[]> {
    const now = Date.now();
 ...
 */
export async function executeDynamicTool(name: string, args: any, meta?: { chatwootId?: string }): Promise<string> {
    ...
    if (registration.client) {
        ...
        try {
            ...
            if (name === 'mcp_enviacardapio') actualToolName = 'mcp_cardapio';
